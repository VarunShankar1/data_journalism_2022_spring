...
**Answer**
Amount is dbl, name is chr, id is dbl
```{r}
...
**Answer**:
135073, they are the same numbers. By that I can conclude that those aren't necesarilly data mistakes, they're intentional entries that have NA designations  
```{r}
ppp_maryland_loans %>%
...
**Answer**
Largest is 10000000, smallest is 6. That is suspicious, because those numbers are illogical, it doesn't feel like anyone would be approved for a loan as big as the max or be approved for a loan as small as the minimum.
```{r}
...
**Answer**
inconsistent style for headers
duplicated row
inconsistent data entries, particularly with city and street address
```{r}
arnold_md_loans
```
...
**Answer**
it added an x at the beginning
...
I think it means that there are mismatched data types that are messing up the amount
```{r}
# cleaning function
...
**Answer**
there are two, the business name is Stuart Jones LLC
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
...
**Answer**
Standardizes the zip codes to five digits
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
