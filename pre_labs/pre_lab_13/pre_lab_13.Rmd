...
**Answer**
State name, vote in 2016, percent that went to trump, and PPE percent
```{r}
...
**Answer**
A pretty strong linear relationship
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
A slightly lesser linear correlation.
```{r}
...
**Answer**
The R value for businesses receiving ppe pct is about .522 and the R value for ppe amount given per employee was about .221. The former is more strongly correlated w the Trump vote than the latter.
```{r}
reveal_data %>%
...
**Answer**
It does a T-test, finding the t value along with the degrees of freedom along with the p-value. There is also a 95 percent confidence interval. It gives us a correlation value of .521. The low p value should mean you reject the null hypothesis and that data supports the alternative hypothesis.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
Similar to above, however this time the p value is higher meaning you don't reject the null hypothesis. The R value is lower here.
```{r}
...
**Answer**
AVerage ppp amount per employee for clinton and trump voting states, and mean business receiving ppe amount for Clinton and Trump voting states
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
p value is lower than .05, so this shows that the true difference in means between Trump and Clinton is different, such as the alternative hypothesis says. 
```{r}
