...
**Answer**
A lot of data about COVID vaccinations, including daily doses and cumulative doses per day per dose.
```{r}
...
**Answer**
They're all almost the same for each county even though there are clear population differencesLet's see if there's anything unusual in the data by counting the number of rows for each county:
...
**Answer**
They were doses that weren't administered in places that they noted the county.
...
**Answer**
It shows vaccines administered in PG county by day along with the cumulative amount. This shows that the amount of doses being given out is dropping.
```{r}
...
**Answer**
There's been a steady drop in the number of people who are getting vaccinated (partly because it's not something you do multiple times)
```{r fig.width=9}
ggplot() +
...
**Answer**
COVID cases per county, cumulative over time
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
Same as above but with a better scale.
```{r}
...
**Answer**
Shows covid cases on 4/19/22 on the map with a magma scale.
```{r}
ggplot() +
