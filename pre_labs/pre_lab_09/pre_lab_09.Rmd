...
**Answer**
1 - storing data in dataframe
2 - grouping by the county's name
3 - summarizing the total # of loans per county
4 - Arranging counties from most to least loans
5 - showing the top 10
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
Bar graphs with each county's loans
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
Added a blue color to the background
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
Added a title, renamed x and y axes, and added a caption.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Loans sorted by the date they were approved.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A line graph that tracks the loans approved on each day
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
It adds markings on the x-axis with the dates
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Turns the labels 45 degrees and makes them easier to read.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
