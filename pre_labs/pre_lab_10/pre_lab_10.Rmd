...
https://datawrapper.dwcdn.net/yhwtb/1/
...
Grouping Maryland PPP loans by county name
```{r}
...
**Answer** Describe what the code below is doing and the output.
filtering out any groups with less than 6 loans
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
code for maryland state and county put together.
```{r}
...
County names, PPP loans for each, and the county's FIP code.
```{r}
...
**Answer** Describe the changes from the previous map
Hovering now gives county name and number of loans.
...
https://datawrapper.dwcdn.net/wYOac/1/
