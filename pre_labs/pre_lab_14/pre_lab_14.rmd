...
**Answer**
Only detects unique words, but detecs different cases as different words
```{r}
...
**Answer**
Lists all unique words used in tweets
```{r}
...
**Answer**
Not really, these are just filler words.
...
**Answer**
Yes, these are nouns/verbs, non stop words
```{r}
unique_words %>%
...
**Answer**
Splits the dataframe into two for 2020 and 2021.
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Similar table to aabove but pairs two words together in a short phrase.
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
Similar to 2020 but words involving vaccine are higher up.
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
These are actually very similar, more than I'd expect them to be. 
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
Republican house candidates talked about Trump a lot before 2021. 
```{r}
covid_tweets %>%
...
**Answer**
Trump being positive seems incorrect, although they probably link it to the verb rather than the name. Positive may also not be positive if COVID is involved.
```{r}
