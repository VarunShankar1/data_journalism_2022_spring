...
**Answer**
List of two digit NAICS codes and the industries they correspond to
### Task 3: Load image
...
**Answer**
list of head and body HTML elements on webpage
```{r}
# read in the html
...
**Answer**
Two subsections, head and body tags
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
Second row looks like the one we're looking for
...
**Answer**
The second data table looks to have the things we need to scrape
### Task 14: Load image
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**
a data table with sector and description (sector being the two-digit NAICS code)
### Task 16: Load image
...
**Answer**
Eliminated row 21 and cleaned the names of the columns
```{r}
# Read in all html from table, store all tables on page as nested list of dataframes.
naics_industry <- naics_url %>%
  read_html() %>%
...
**Answer**
dataframe with sector code and name
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Shows table with value for number of employees.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**
two items in a list, head and body
```{r}
...
**Answer**
shows the table with id iag22emp1 as a list
```{r}
...
**Answer**
two column table with just one row of data, with employment and the employment number.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Page with employment data for mining, same format as page for utilities
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
Page with employment data for construction, same format as page for utilities
...
**Answer**
have a list of sectors with all the sectors stored inside
```{r}
...
**Answer**
printed all the sectors from the list using for loop
```{r}
...
**Answer**
error, object sector list not found
...
**Answer**
dataframe with employment for mining
```{r}
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**
all NAICS two digit sector codes
```{r}
# Define url of page we want to scrape
...
**Answer**
the sectors, the descriptions, the URLs, and the xpathID
```{r}
...
**Answer**
same as above without public administration
```{r}
...
**Answer**
separate table for each dataframe
```{r}
...
**Answer**
printed the row number and new url and xpath for all the rows
```{r}
...
**Answer**
Got tables for all the naics industries that shows all the employment data (some extraneous stuff)
```{r}
...
**Answer**
shows all the data but shows the data we need along with clean names
```{r}
...
**Answer**
joins the employment data with the corresponding naics code
```{r}
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**
used select to keep the column of only the nov 2021 data
```{r}
...
**Answer**
dataframe with the number of employees in november 2021 for each sector.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**
agriculture page for employment
...
**Answer**
replaces agriculture value with NA
```{r}
