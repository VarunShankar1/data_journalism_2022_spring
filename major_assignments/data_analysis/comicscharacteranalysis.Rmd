...
- LGBT characters filtered by pre 2000 and post 2000
- test that by seeing if how average app and >1 app change before and after 2000
```{r}
...
total_characters %>% 
    filter(appearances > 1 & year > 2000) %>% 
    summarise(count=n(), pct = count/) %>%
    arrange(desc(count))
total_characters %>% 
    filter(appearances > 1 & year < 2000) %>% 
    summarise(count=n()) %>%
...
- how does this change for nonlgbt characters pre post 2000
- how does the value change just across gender
```{r}
